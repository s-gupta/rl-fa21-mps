...
class CartPoleEnv(gym.Env):
    ...
    def get_observation(self):
        if self.visual:
            if self.png:
                ...
                out = np.array(Image.fromarray(self.render(mode='rgb_array')).resize((160,210)))
            # out = np.array(Image.fromarray(self.render(mode='rgb_array')).transpose(Image.ROTATE_90).resize((160,210)))
            # out = np.array(Image.fromarray(self.render(mode='rgb_array')))
        else:
    ...
    def reset(self):
        noise_level = 0.05
        # noise_level = 0.5
        self.state = self.np_random.uniform(low=-noise_level, high=noise_level, size=(4,))
        self.steps_beyond_done = None
        self.upright_frames = 0
        self.total_fames = 0
